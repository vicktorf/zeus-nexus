"""
Zeus Nexus Ast.set_page_config(
    page_title="Zeus Nexus",
    page_icon="⚡",
    layout="wide",
    initial_sidebar_state="expanded"
)

__version__ = "2.3.2"in Application
...
with st.sidebar:
    ...
    if True:  # Always show sidebar
        ...
        if st.session_state.current_page == "chat":
            ...
            # New Chat Button
            if st.button("➕ New Chat", use_container_width=True, type="primary", help="Start a new conversation"):
                st.session_state.messages = []
                st.session_state.session_id = None
            ...
            try:
                response = requests.get(f"{ZEUS_API_URL}/conversations", timeout=5)
                if response.status_code == 200:
                    conversations = response.json().get("conversations", [])
                    if conversations:
                        for conv in conversations[:5]:  # Show last 5
                            # Create a short title from first message or use session_id
                            title = conv.get("first_message", conv.get("session_id", "Chat"))[:30]
                            if st.button(f"💬 {title}...", key=f"hist_{conv['session_id']}", use_container_width=True):
                                st.session_state.session_id = conv["session_id"]
                                # Load messages from this conversation
                                st.info(f"Loading conversation: {conv['session_id']}")
                                st.rerun()
                    else:
...
if st.session_state.current_page == "llm_setup":
    ...
    # Model selector - full width above chat input
    st.markdown("---")
    
    if st.session_state.available_models:
        model_options = [m["model"] for m in st.session_state.available_models if m.get("api_key_configured", False)]
        
        if model_options:
            # Get provider icons
            provider_icons = {"openai": "🤖", "anthropic": "🧠", "google": "🔍"}
            
            # Format options with provider icon
            def format_model(model_id):
                provider = next((m["provider"] for m in st.session_state.available_models if m["model"] == model_id), "")
                icon = provider_icons.get(provider, "🤖")
                # Full model name display
                return f"{icon} {model_id}"
            
            selected_index = model_options.index(st.session_state.selected_llm) if st.session_state.selected_llm in model_options else 0
            ...
            new_model = st.selectbox(
                ...
                key="model_selector_input",
                help="Choose the LLM model for your conversation"
            )
            
            if new_model != st.session_state.selected_llm:
                st.session_state.selected_llm = new_model
                # Save last used model to database
                try:
                    provider = next((m["provider"] for m in st.session_state.available_models if m["model"] == new_model), "")
                    settings_data = {"last_chat_model": new_model, "last_chat_provider": provider}
                    requests.post(
                    ...
                    st.toast(f"✅ Switched to {new_model}", icon="🤖")
                except:
                    pass  # Silent fail for settings save
        else:
            st.warning("⚠️ No LLM models configured. Please configure API keys in Settings.", icon="⚠️")
            if st.button("🔧 Go to LLM Setup"):
                st.session_state.current_page = "llm_setup"
                st.rerun()
    else:
        st.info("⚙️ Please configure LLM providers in Settings to start chatting.", icon="ℹ️")
        if st.button("🔧 Configure LLM"):
            st.session_state.current_page = "llm_setup"
            st.rerun()
    
    # Chat input (must be outside any container)
    prompt = st.chat_input("Ask Zeus anything...")
